---
title: "Data sampling"
author: "Steph Locke, modified by Lee-Anne Cecchini"
...
You ideally want the class (Yes/No) distribution to be similar in training and testing data.
```{r samplerep}
...
For reproducibility, you need to set the seed, then check that your sample satisfies the similarity criteria (from the same distribution)
```{r setseed}
pass_ttest <- function(x, prop_train = .07) {
  set.seed(6767)
  n_x_all <- length(x)
...
Using caret package: helps with data selection, feature prep and other useful functions.

```{r caretsample}
library(caret)
rows_train_c <-
createDataPartition(titanic_all$survived, p = prop_train, list = FALSE)
